...
for iev, event in enumerate(tree):
    ...
    for icl_noseed in nonseed_clusters:
        ...
        for window in windows_map:
            isin = cluster_in_window(window, clxtals_ieta, clxtals_iphi, clxtals_iz, clxtals_energy)
            if isin:
                clusters_event.append({
                    "window_index": window["metadata"]["index"],
                    "cluster_deta": cl_eta - window["metadata"]["seed_eta"],
