...
class SelfAttention(tf.keras.layers.Layer):
    ...
    def call(self, x, mask, training):
        ...
        sa_output = scaled_dot_product_attention(q, k, v, mask_for_attention)
        # Dropout
...
class SelfAttentionBlock(tf.keras.layers.Layer):
    ...
    def call(self, x, mask, training):
        ...
        # Now the aggregation 
        if self.reduce == "sum":
            return  tf.reduce_sum(output_block, -2)
        if self.reduce == "mean":
            N_nodes = tf.reduce_sum(mask,-1)[:,tf.newaxis]
            return tf.math.divide_no_nan( tf.reduce_sum(output_block, -2), N_nodes)
        if self.reduce == "max":
            return tf.reduce_max(output_block, axis=-2)
        else:
...
class RechitsGCN(tf.keras.layers.Layer):
    ...
    def call(self, x, mask, training):
        ...
        sa_output = self.drop1(sa_output, training=training)
        sa_output = self.sa_normalization(sa_output, training=training) * mask_for_output
        ...
        # Add + Norm
        dense_output = self.out_normalization(dense_output + sa_output, training=training) * mask_for_output
...
class GraphBuilding(tf.keras.layers.Layer):
    ...
    def call(self, cl_features, rechits_features, training):
        ...
        #rechits = rechits_features.to_tensor() ----> Already converted
        rechits = rechits_features
        mask_rechits, mask_cls = create_padding_masks(rechits)
        # Cal the rechitGCN and get out 1 vector for each cluster 
        output_rechits = self.rechitsGCN(rechits, mask_rechits, training=training)
        ...
        cl_and_rechits = self.feat_layer_normalization(cl_and_rechits, training=training) * mask_cls[:,:,tf.newaxis]

        #cl_and_rechits are now the baseline node features
        
        # Now apply the coordinate network with a layer normalization before
        coord_output = self.SA_coord(cl_and_rechits, mask_cls, training)
        ...
        return  cl_and_rechits, mask_cls, adj # coord_output, adj, mask_cls, output_rechits, coord_att_ws
...
class DeepClusterGN(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        #removed almost all the other outputs of graphbuild
        cl_X, mask_cls, adj = self.graphbuild(cl_X_initial, cl_hits, training)
        ...
        out_SA_clclass = self.SA_clclass(out_gcn, mask_cls, training)
        ...
        out_SA_windcl = self.SA_windclass(out_SA_and_inputs, mask_cls, training)
        ...
        out_SA_windcl = self.SA_windclass_layernorm(out_SA_windcl, training=training)
        ...
        #input_en_regr = self.SA_enregr_layernorm(input_en_regr, training=training)
        #out_SA_enregr = self.SA_enregr(input_en_regr, mask_cls, training)
	...
	'''
        return { "cl_class" : clclass_out, "wind_class": windclass_out}
         # "en_regr":  out_SA_enregr, "mask_cls":mask_cls
