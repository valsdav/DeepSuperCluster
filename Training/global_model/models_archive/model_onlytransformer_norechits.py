...
class FeaturesBuilding(tf.keras.layers.Layer):
    ...
    def __init__(self,  **kwargs):
        ...
        #NOT NEEDED     self.output_dim_rechits = kwargs.pop("output_dim_rechits",16)
        self.output_dim_features = kwargs.pop("output_dim_features",32)
        #NOT NEEDED     self.rechit_num_transf_layers = kwargs.pop("rechit_num_transf_layers", 2)
        #NOT NEEDED     self.rechit_num_transf_heads = kwargs.pop("rechit_num_transf_heads", 4)
        #NOT NEEDED     self.rechit_transf_ff_dim = kwargs.pop("rechit_transf_ff_dim", 64)

        self.dropout = kwargs.get("dropout", 0.)
        self.l2_reg = kwargs.get("l2_reg", False)
        name = kwargs.get("name", None)
            
        #NOT NEEDED     self.rechits_transf = RechitsTransformer(name="rechit_transformer",
    ...
    def get_config(self):
        return {
            ...
            #NOT NEEDED     "output_dim_rechits": self.output_dim_rechits,
            "output_dim_features": self.output_dim_features,
            #NOT NEEDED     "rechit_num_transf_layers": self.rechit_num_transf_layers,
            #NOT NEEDED     "rechit_num_transf_heads": self.rechit_num_transf_heads,
            #NOT NEEDED     "rechit_transf_ff_dim": self.rechit_transf_ff_dim,
            "output_dim": self.output_dim,
            "l2_reg": self.l2_reg,
            "dropout": self.dropout,
            "activation": self.activation,
    ...
    #DELETED SELF, CL_FEATURES, RECHITS_FEATURES, MASK_RECHITS, ..
    def call(self, cl_features, mask_cls, training):
        # Cal the rechitGCN and get out 1 vector for each cluster 
        #NOT NEEDED     output_rechits = self.rechits_transf(rechits_features, mask_rechits, training=training)
...
class DeepClusterGN(tf.keras.Model):
    ...
    Model parameters:
    - activation
    NOT NEEDED          - output_dim_rechits:  latent space dimension for the rechits per-cluster feature vector
    ...
    NOT NEEDED          - rechit_num_transf_layers: number of transformer layers for rechits feature extraction
    NOT NEEDED          - rechit_num_transf_heads: number of transformer heads in the rechits transformer
    NOT NEEDED          - rechit_transf_ff_dim: hidden dimention of the transformer for the rechits
    ...
    ADAPT?              - layers_input:  list representing the DNN applied on the [rechit+cluster] concatened features to build the clusters latent space
    ...
    def __init__(self, **kwargs):
        ...
        #NOT NEEDED     self.rechit_num_transf_layers = kwargs.get("rechit_num_transf_layers", 2)
        #NOT NEEDED     self.rechit_num_transf_heads = kwargs.get("rechit_num_transf_heads", 4)
        #NOT NEEDED     self.rechit_transf_ff_dim = kwargs.get("rechit_transf_ff_dim", 64)
    ...
    def get_config(self):
        return {
            ...
            #NOT NEEDED     "rechits_num_transf_layers": self.feature_building.rechit_num_transf_layers,
            #NOT NEEDED     "rechits_num_transf_heads": self.feature_building.rechit_num_transf_heads,
            #NOT NEEDED     "rechits_transf_ff_dim": self.feature_building.rechit_transf_ff_dim,
            ...
            #NOT NEEDED     "output_dim_rechits": self.feature_building.output_dim_rechits,
            "output_dim_features": self.output_dim_features,
