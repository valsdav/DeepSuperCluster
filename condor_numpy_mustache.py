...
# Prepare condor jobs
condor = '''executable              = run_numpy_script_mustache.sh
output                  = output/strips.$(ClusterId).$(ProcId).out
error                   = error/strips.$(ClusterId).$(ProcId).err
log                     = log/strips.$(ClusterId).log
transfer_input_files    = cluster_tonumpy_mustache.py, windows_creator_mustache.py
...
python cluster_tonumpy_mustache.py -i ${INPUTFILE} -o output.pkl --weta ${WETA_EB} ${WETA_EE}\
...
with open("run_numpy_script_mustache.sh", "w") as rs:
    rs.write(script)
