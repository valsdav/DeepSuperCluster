...
class WindowCreator():
    ...
    def get_windows(self, event, assoc_strategy,  nocalowNmax=0, min_et_seed=1, debug=False):
        # Branches
                ...
                if isin:
                    ...
                    if not window["metadata"]["is_seed_calo_matched"]:
                        ...
                        if is_calo_matched: 
                            ...
                            in_scluster = self.pass_simfraction_threshold(window["metadata"]["seed_eta"], 
        ...
        # Now we need to convert list of clusters metadata in list
        output_data = []

        for window in windows_to_keep:
            outw = { "window_index": window["window_index"]}
            outw.update(window["metadata"])
